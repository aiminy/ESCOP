...
char *res_id;
double ave_n_d;
double x,y,z,sc_x,sc_y,sc_z,angle;
} target;
...
target t;
vector<neighbor> n;
vector<neighbor> sorted_n;
int t_id,n1_id,n2_id,n3_id;
} target_many_neighbor;

typedef struct{
int a,b,c,d;
double p,count,angle;
} four_nu;
...
void inter_atom_distance_matrix(vector<aa> &w_c,vector<aa> &sp,vector<target_many_neighbor> &t_m_n_chain_for_all_protein);
...
void sum_for_closest_four_atom_chain(char *file_name,vector<target_many_neighbor>&t_m_n_chain_for_all_protein);
